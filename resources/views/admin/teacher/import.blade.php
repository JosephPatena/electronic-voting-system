	    ...
	    <div class="row mb-2">
	      ...
	      <div class="col-sm-6">
	        <ol class="breadcrumb float-sm-right">
	          ...
	          <li class="breadcrumb-item active">Import Teacher</li>
	        </ol>
	      </div>
	    </div>
	  </div><!-- /.container-fluid -->
	              	...
	              	@foreach($imported as $value)
                        ...
                        {!! empty($value->user) && $value->is_invited ? '<span class="badge bg-primary">Invited</span>' : '' !!}
                            ...
                            <div role="menu" class="dropdown-menu">
                                <a href="{{ route('invite_teachers', $value->id) }}" class="dropdown-item">Send Invitation</a>
                                @if(empty($value->user))
...
@section('scripts')
    ...
    <script src="{{ asset('plugins/dropzone/min/dropzone.min.js') }}"></script>
      ...
      var myDropzone = new Dropzone(document.body, { // Make the whole body a dropzone
        url: "{{ route('teachers.store') }}", // Set the url
        thumbnailWidth: 80,
        thumbnailHeight: 80,
        parallelUploads: 20,
