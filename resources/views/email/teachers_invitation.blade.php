...
<div style="margin:0px auto;max-width:640px;background:transparent;"><table role="presentation" cellpadding="0" cellspacing="0" style="font-size:0px;width:100%;background:transparent;" align="center" border="0"><tbody><tr><td style="text-align:center;vertical-align:top;direction:ltr;font-size:0px;padding:40px 0px;"><!--[if mso | IE]>
      ...
      <![endif]--><div style="cursor:auto;color:white;font-family:Whitney, Helvetica Neue, Helvetica, Arial, Lucida Grande, sans-serif;font-size:36px;font-weight:600;line-height:36px;text-align:center;">Join us at {{ Helper::get_active_election()->name }}<br><span style="font-size:20px;font-weight:300;">{{ \Carbon\Carbon::parse(Helper::get_active_election()->date_start)->format('F d, Y h:i A'). " to " .\Carbon\Carbon::parse(Helper::get_active_election()->date_end)->format('F d, Y h:i A') }}</span></div><!--[if mso | IE]>
      </td></tr></table>
  ...
  <p>
    You are invited to participate to an Online Election which going to start on <span style="color: #dd571c;">{{ \Carbon\Carbon::parse(Helper::get_active_election()->date_start)->format('F d, Y h:i A'). " to " .\Carbon\Carbon::parse(Helper::get_active_election()->date_end)->format('F d, Y h:i A') }}</span>. Please click the button below to register and participate to this said event.
    <br><br>
    Looking forward for your participation at this event!
    <br><br>
    Regards,
    <br>
    The EVS Team
            ...
            <a href="{{ route('teachers_registration', encrypt($key)) }}" style="text-decoration:none;line-height:100%;background:#7289DA;color:white;font-family:Ubuntu, Helvetica, Arial, sans-serif;font-size:15px;font-weight:normal;text-transform:none;margin:0px;" target="_blank">
            Register
      ...
      Sent by EVS • <a href="/" style="color:#1EB0F4;text-decoration:none;" target="_blank">Electronic Voting System | EVS</a>
